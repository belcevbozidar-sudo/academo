...
export default function AddNonSchoolDayPage() {
  ...
  const createNonSchoolDay = useMutation(api.nonSchoolDays.createNonSchoolDay);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Неучебният ден е създаден успешно");
      navigate("/bg/admin/non-school-days");
    } catch (error) {
      toast.error("Грешка при създаване на неучебен ден");
    }
  };
    ...
    <Layout>
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">Добави неучебен ден</h1>
          <Button variant="outline" onClick={() => navigate("/bg/admin/non-school-days")}>
            Отказ
          </Button>
        </div>
        ...
        {step === 1 ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Select value={category} onValueChange={setCategory}>
                  <SelectTrigger>
                    <SelectValue placeholder="Ваканция" />
                  </SelectTrigger>
                  <SelectContent>
                    {categories.map((cat) => (
          ...
          <div className="space-y-6 bg-background border rounded-lg p-6">
            ...
            <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-4 flex gap-3">
              <AlertTriangleIcon className="h-6 w-6 text-yellow-600 shrink-0 mt-0.5" />
              <div className="space-y-1">
                <p className="font-medium text-yellow-900">ВНИМАНИЕ:</p>
                <p className="text-sm text-yellow-800">
                  Ще бъдат премахнати всички{" "}
                  <strong>
                    занятия за период ({startDate} - {endDate})
                  </strong>
                  , както и всички{" "}
                  <strong>теми на часа, занятия по заместване и отсъствия</strong>,
                  свързани с тях.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="font-medium text-lg">Данни за изтриване</h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="font-medium">Тип данни</div>
                </div>
                <div>
                  <div className="font-medium">Брой</div>
                </div>
              </div>
            </div>
