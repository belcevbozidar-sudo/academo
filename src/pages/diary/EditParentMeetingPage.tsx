...
function EditParentMeetingPageInner() {
  const navigate = useNavigate();
  const { classId, meetingId, lng } = useParams<{ classId: string; meetingId: string; lng: string }>();
  ...
  const updateMeeting = useMutation(api.parentMeetings.update);

  // Load existing meeting data
  useEffect(() => {
    if (meetings && meetingId && !isLoaded) {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateMeeting({
        id: meetingId as Id<"parentMeetings">,
        title: title.trim(),
      ...
      toast.success("Родителската среща е обновена успешно");
    ...
    if (!meeting) {
      return (
        <div className="text-center py-12">
          <p className="text-muted-foreground">Родителската среща не е намерена.</p>
          <Button
            variant="outline"
            className="mt-4"
...
export default function EditParentMeetingPage() {
  return (
    <>
      <AuthLoading>
