...
function EditEventInner() {
  ...
  // Initialize form from existing event
  useEffect(() => {
    if (existingEvent && !initialized) {
      setTitle(existingEvent.title);
      setDescription(existingEvent.description || "");
      setIsPaid(existingEvent.isPaid || false);
      setIsSchoolCalendar(existingEvent.isSchoolCalendar || false);
      setCategory(existingEvent.category);
      setStartDate(toLocalDatetimeString(existingEvent.startDate));
      setEndDate(toLocalDatetimeString(existingEvent.endDate));
      if (existingEvent.registrationDeadline) {
        setRegistrationDeadline(
          toLocalDatetimeString(existingEvent.registrationDeadline)
        );
      }
      setMinRegistrants(
        existingEvent.minRegistrants?.toString() || ""
      );
      setMaxRegistrants(
        existingEvent.maxRegistrants?.toString() || ""
      );
      setSelectedUsers(new Set(existingEvent.invitedUserIds));
      setLocationAddress(existingEvent.locationAddress || "");
      setLocationCity(existingEvent.locationCity || "");
      setLocationDescription(existingEvent.locationDescription || "");

      // Populate existing files (we only have IDs, use index as name)
      if (existingEvent.fileIds) {
        setUploadedFiles(
          existingEvent.fileIds.map((id, i) => ({
            storageId: id,
            name: `Файл ${i + 1}`,
          }))
        );
  ...
  const usersForInvite = useQuery(api.events.listUsersForInvite, {
    classId:
      inviteClassFilter !== "all"
        ? (inviteClassFilter as Id<"classes">)
        : undefined,
  ...
  if (!currentUser || !classes || existingEvent === undefined) {
    return (
  ...
  if (existingEvent === null) {
    return (
      <div className="p-6">
        <p className="text-muted-foreground">Събитието не е намерено.</p>
        <Button
          variant="ghost"
          className="mt-4"
