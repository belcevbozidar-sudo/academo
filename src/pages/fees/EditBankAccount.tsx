...
function EditBankAccountInner() {
  const navigate = useNavigate();
  const { lng, id } = useParams<{ lng: string; id: string }>();
  const updateBankAccount = useMutation(api.fees.updateBankAccount);
  const bankAccount = useQuery(api.fees.getBankAccount, { id: id as Id<"bankAccounts"> });
  ...
  useEffect(() => {
    if (bankAccount) {
      ...
      // Check if bank is in the predefined list
      ...
      if (predefinedBanks.includes(bankAccount.bank)) {
        setBank(bankAccount.bank);
      } else {
        setBank("Друга");
        setCustomBank(bankAccount.bank);
      }
    }
  }, [bankAccount]);
      ...
      <Layout>
        <div className="p-6">
          <div className="text-center text-muted-foreground">
            Банковата сметка не е намерена
          </div>
        </div>
      </Layout>
    );
  ...
  const handleSave = async () => {
    ...
    try {
      await updateBankAccount({
        id: id as Id<"bankAccounts">,
        name,
        iban,
        bank: bank === "Друга" ? customBank : bank,
      });

      toast.success("Банковата сметка е актуализирана успешно");
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between border-b pb-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold">Редактиране на банкова сметка</h1>
          </div>
          <div className="flex items-center gap-2">
