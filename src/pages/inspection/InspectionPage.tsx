...
export default function InspectionPage() {
  ...
  const [selectedTeacherId, setSelectedTeacherId] = useState<
    Id<"teachers"> | undefined
  >(undefined);
  const [selectedClassId, setSelectedClassId] = useState<
    Id<"classes"> | undefined
  >(undefined);
  const [clearDialogOpen, setClearDialogOpen] = useState(false);
  const [lessonToClear, setLessonToClear] = useState<Id<"lessons"> | null>(
    null,
  );

  // Mutations for inspection actions
  const markAsTaken = useMutation(api.inspections.markLessonAsTaken);
  const markAsNotConducted = useMutation(
    api.inspections.markLessonAsNotConducted,
  );
  ...
  const teacherRecord = useQuery(
    ...
    currentUser?._id && !isAdminOrDirector
  ...
  const hasAnyLessons =
    monthStats && monthStats.some((m) => m.totalLessons > 0);
  ...
  const handleConfirmClear = async () => {
    ...
    try {
      ...
      toast.success(
        `Изчистени: ${result.deletedAttendance} отсъствия, ${result.deletedGrades} оценки, ${result.deletedBadges} отзива`,
      );
    } catch (error) {
  ...
  const filteredLessons = useMemo(() => {
    ...
    return dayDetails.lessons.filter(
      (l) =>
        !selectedClassId ||
        l.className.includes(
          allClasses?.find((c) => c._id === selectedClassId)?.name ?? "",
        ),
    );
  ...
  const selectedPeriodStats = useMemo(() => {
    if (selectedPeriod === null || selectedPeriodLessons.length === 0)
      return null;
    const totalLessons = selectedPeriodLessons.length;
    const takenLessons = selectedPeriodLessons.filter((l) => l.isTaken).length;
    const totalAbsences = selectedPeriodLessons.reduce(
      (sum, l) => sum + l.absencesCount,
      0,
    );
    const totalGrades = selectedPeriodLessons.reduce(
      (sum, l) => sum + l.gradesCount,
      0,
    );
    const totalRemarks = selectedPeriodLessons.reduce(
      (sum, l) => sum + l.remarksCount,
      0,
    );
    const percentageTaken =
      totalLessons > 0 ? Math.round((takenLessons / totalLessons) * 100) : 0;
  ...
  const takenPercentage = useMemo(() => {
    ...
    return dayDetails.summary.totalLessons > 0
      ? Math.round(
          (dayDetails.summary.takenLessons / dayDetails.summary.totalLessons) *
            100,
        )
      : 0;
  }, [dayDetails]);
          ...
          <Card className="bg-card">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-muted-foreground text-sm">
                Тази страница е достъпна само за администратори, директори и
                заместник-директори.
              </p>
      ...
      <div className="container mx-auto p-6 space-y-4">
        ...
        <div className="flex items-center justify-between border-b border-border pb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {(viewLevel === "details" || viewLevel === "lesson") &&
              selectedDate && (
        ...
        {isAdminOrDirector && (
          <div className="flex items-center gap-4 flex-wrap">
            <Select
              ...
              onValueChange={(v) =>
                setSelectedTeacherId(
                  v === "all" ? undefined : (v as Id<"teachers">),
                )
              }
            >
            ...
            <Select
              ...
              onValueChange={(v) =>
                setSelectedClassId(
                  v === "all" ? undefined : (v as Id<"classes">),
                )
              }
            >
              <SelectTrigger className="w-[200px]">
        ...
        {(viewLevel === "details" || viewLevel === "lesson") && dayDetails && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card
              className={`${getCardColorClasses(takenPercentage)} border-0`}
            >
              <CardContent className="p-4 text-center text-white">
                <p className="text-3xl font-bold">
                  {dayDetails.summary.takenLessons} /{" "}
                  {dayDetails.summary.totalLessons}
                </p>
            ...
            <Card className="bg-amber-500 dark:bg-amber-600 border-0">
              <CardContent className="p-4 text-center text-white">
                <p className="text-3xl font-bold">
                  {dayDetails.summary.totalAbsences}
                </p>
            ...
            <Card className="bg-lime-500 dark:bg-lime-600 border-0">
              <CardContent className="p-4 text-center text-white">
                <p className="text-3xl font-bold">
                  {dayDetails.summary.totalGrades}
                </p>
            ...
            <Card className="bg-purple-500 dark:bg-purple-600 border-0">
              <CardContent className="p-4 text-center text-white">
                <p className="text-3xl font-bold">
                  {dayDetails.summary.totalRemarks}
                </p>
        ...
        {viewLevel === "lesson" && selectedPeriodStats && (
          <div
            className={`${periodColorClasses.split(" hover:")[0]} border rounded-lg p-3 flex items-center justify-between`}
          >
            <div
              className={`flex items-center gap-2 ${periodColorClasses.split(" ").find((c) => c.startsWith("text-"))}`}
            >
              <button
                onClick={handleClosePeriodDetails}
                className={`${periodColorClasses
                  .split(" ")
                  .filter((c) => c.startsWith("hover:"))
                  .join(" ")} rounded p-1 transition-colors`}
              >
                <X className="h-4 w-4" />
              </button>
              <span className="font-medium">
                Час #{selectedPeriodStats.periodIndex} (
                {selectedPeriodStats.startTime}-{selectedPeriodStats.endTime}) -{" "}
                {selectedPeriodStats.percentageTaken}% взет,{" "}
                {selectedPeriodStats.totalAbsences} отсъствия,{" "}
                {selectedPeriodStats.totalGrades} оценки,{" "}
                {selectedPeriodStats.totalRemarks} отзива
              </span>
            </div>
          </div>
        )}
            ...
            {!hasAnyLessons ? (
              <Card className="bg-card">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground text-sm mt-2">
                    Данните ще се появят след като бъдат въведени часове в
                    дневника.
                  </p>
                </CardContent>
              </Card>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {monthStats.map((stat) => (
                  ...
                  >
                    <Card
                      className={`${getCardColorClasses(stat.percentageTaken)} border-0 hover:opacity-90 transition-opacity`}
                    >
                      <CardContent className="p-6">
                        <h3 className="text-xl font-bold mb-2">
                          {MONTH_NAMES_BG[stat.month - 1]}
                        </h3>
                        <p className="text-lg font-semibold">
                          {stat.percentageTaken}% взет
                        </p>
                        <p className="text-sm opacity-80">
                          {stat.takenLessons}/{stat.totalLessons} часа
                        </p>
                      </CardContent>
                    </Card>
                  </button>
        ...
        {viewLevel === "days" && dayStats && (
          <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-7 gap-3">
            {dayStats.map((stat) => (
              ...
              >
                <Card
                  className={`${stat.totalLessons === 0 ? "bg-muted text-muted-foreground" : getCardColorClasses(stat.percentageTaken)} border-0 hover:opacity-90 transition-opacity`}
                >
          ...
          <div className="space-y-2">
            {Object.entries(lessonsByPeriod)
              ...
              .map(([period, lessons]) => {
                ...
                const totalAbsences = lessons.reduce(
                  (sum, l) => sum + l.absencesCount,
                  0,
                );
                const totalGrades = lessons.reduce(
                  (sum, l) => sum + l.gradesCount,
                  0,
                );
                const totalRemarks = lessons.reduce(
                  (sum, l) => sum + l.remarksCount,
                  0,
                );
                const percentageTaken =
                  totalLessons > 0
                    ? Math.round((takenLessons / totalLessons) * 100)
                    : 0;
                const firstLesson = lessons[0];

                // Determine color based on percentage
                const periodCardClasses =
                  percentageTaken === 100
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <p
                              className={`text-sm font-semibold ${periodTextClasses}`}
                            >
                              Час #{periodNum} ({firstLesson?.startTime}-
                              {firstLesson?.endTime}) - {percentageTaken}% взет,{" "}
                              {totalAbsences} отсъствия, {totalGrades} оценки,{" "}
                              {totalRemarks} отзива
                            </p>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {selectedPeriodLessons.map((lesson) => {
                        ...
                        const formattedTeacherNames = (
                          lesson.teacherNames ?? [lesson.teacherName]
                        )
                          .filter((n) => n && n !== "Unknown")
                          .map((n) => `б/д ${formatFullName(n)}`);
                        const teacherDisplay =
                          formattedTeacherNames.length > 0
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell className="text-center text-foreground">
                              {/* Late arrivals - would need to be added */}0
                            </TableCell>
                            <TableCell className="text-center text-foreground">
                              {lesson.gradesCount}
                            </TableCell>
        ...
        <AlertDialog open={clearDialogOpen} onOpenChange={setClearDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                <span className="text-foreground font-medium">Внимание!</span>{" "}
                Това действие ще изтрие всички данни за този час:
